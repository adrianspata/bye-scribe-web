...
interface SubscriptionNode {
  ...
  color: string;
  iconBg: string;
  xPercent: number; // For desktop SVG coordinate (0-1000)
  yPercent: number; // For desktop SVG coordinate (0-550)
  desktopStyle: {
    top?: string;
    bottom?: string;
    left?: string;
...
const SUBSCRIPTIONS: SubscriptionNode[] = [
  ...
  },
  {
    ...
    xPercent: 830,
    yPercent: 390,
    desktopStyle: { bottom: '12%', right: '8%' },
    animationClass: 'animate-float-1',
    icon: (
      <svg viewBox="0 0 24 24" className="w-5 h-5 fill-white" aria-hidden="true">
...
// Target center accumulator node coordinate in 1000x550 SVG space
const CENTER_X = 500;
const CENTER_Y = 470;
...
export function SubscriptionMindmap() {
  ...
  // Baseline sums:
  // Monthly: $145.20
  // 1-Year Baseline: $1,742.40
  const monthlyTotal = SUBSCRIPTIONS.reduce((acc, curr) => acc + curr.price, 0);
  const annualTotal = monthlyTotal * 12;

  // Real-time ticking accumulator that starts at annualTotal and smoothly counts up continuously
  const [accumulatedCents, setAccumulatedCents] = useState(annualTotal);

  useEffect(() => {
    // Accumulate ~ $0.08 every 120ms to simulate the relentless real-time leakage of subscription fees
    const interval = setInterval(() => {
      setAccumulatedCents((prev) => prev + 0.08);
    }, 120);
      ...
      <div className="relative w-full h-[460px] sm:h-[500px] lg:h-[520px] mt-2 hidden md:block select-none">
        ...
        >
          ...
          {SUBSCRIPTIONS.map((sub) => {
            ...
            const cx1 = sub.xPercent + (CENTER_X - sub.xPercent) * 0.1;
            const cy1 = sub.yPercent + (CENTER_Y - sub.yPercent) * 0.7;
            const cx2 = sub.xPercent + (CENTER_X - sub.xPercent) * 0.8;
            const cy2 = CENTER_Y - 40;
            const pathData = `M ${sub.xPercent} ${sub.yPercent} C ${cx1} ${cy1}, ${cx2} ${cy2}, ${CENTER_X} ${CENTER_Y}`;
